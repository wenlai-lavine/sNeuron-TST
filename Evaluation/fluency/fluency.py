...
def calc_gpt_ppl(args, preds, aggregate=True):
    ...
    gpt_model = GPT2LMHeadModel.from_pretrained(args.model_path).cuda()
    gpt_tokenizer = GPT2Tokenizer.from_pretrained(args.model_path)
    gpt_model.eval()
    ...
    with torch.no_grad():
        for sent in tqdm.tqdm(preds):
            ...
            inp = gpt_tokenizer(sent, truncation=True, max_length=512, return_tensors="pt").input_ids.cuda()

            try:
                result = gpt_model(inp, labels=inp, return_dict=True)
                loss = result.loss.item()
            except Exception as e:
                print(f'Got exception "{e}" when calculating gpt perplexity for sentence "{sent}" ({inp})')
                loss = 100
            
            gpt_ppl.append(100 if np.isnan(loss) or loss >= 10 else math.exp(loss))
    
    if aggregate:
        avg_ppl = sum(gpt_ppl) / len(gpt_ppl)
    return gpt_ppl, avg_ppl
...
def do_cola_eval(args, preds, soft=False):
    ...
    path_to_data = os.path.join(args.model_path, 'cola-bin')

    cola_roberta = RobertaModel.from_pretrained(args.model_path,
